"""
Experiment 6: Training Pipeline for 2000 Stocks
...
def load_and_prepare_2000(sp500_path: str = "experiment_6/data/SP500_daily.csv",
                          stocks_path: str = "experiment_6/data/stocks_2000_daily.parquet",
                          max_symbols: int = 2000,
                          chunk_size: int = 200):
    """
    Load data in chunks to manage memory for 2000 stocks.
    Processes symbols in batches, computes indicators, and merges.
    """
    print("=" * 70)
    print(f"Experiment 6: 2000-Stock Enterprise RL Agent")
    print("=" * 70)
    ...
    # Merge SPX regime (normalize dates first)
    ...
    print(f"  Merged: sp500_close valid: {st_df['sp500_close'].notna().sum():,} / {len(st_df):,}")

    # Process in chunks
    symbols = sorted(st_df["Symbol"].unique())[:max_symbols]
    n_chunks = (len(symbols) + chunk_size - 1) // chunk_size

    print(f"[Indicators] Processing {len(symbols)} symbols in {n_chunks} chunks of {chunk_size}...")

    all_chunks = []
    squeeze_stats = {"moderate": 0, "full": 0, "total": 0}
    ...
    for i in range(n_chunks):
        ...
        if results:
            ...
            if "squeeze_signal" in chunk_df.columns:
                squeeze_stats["moderate"] += (chunk_df["squeeze_signal"] == 1).sum()
                squeeze_stats["full"] += (chunk_df["squeeze_signal"] == 2).sum()
                squeeze_stats["total"] += len(chunk_df)
    ...
    print(f"  Squeeze: Moderate={squeeze_stats['moderate']:,} "
          f"Full={squeeze_stats['full']:,} "
          f"({100*(squeeze_stats['moderate']+squeeze_stats['full'])/max(squeeze_stats['total'],1):.1f}% of days)")

    return combined, fc


def split_data_2000(df, train_ratio=0.7, val_ratio=0.15):
    """Split data per-symbol for 70/15/15 train/val/test."""
    ...
    # Shuffle symbols within each split for better training
    for df_part in [train_df, val_df, test_df]:
        pass  # Keep time order per symbol
...
def evaluate_full(model, eval_df, feature_cols, mean, std, max_steps=5000):
    """Full evaluation pass returning equity curve and trades."""
    env = DummyVecEnv([lambda: EnterpriseTradingEnv(
...
def compute_metrics_2000(eq_curve, trades, initial=100000.0):
...
def train_2000_pipeline(
    run_optuna: bool = False,
    n_optuna_trials: int = 20,
    pretrain_steps: int = 10000,
    finetune_steps: int = 200000,
    max_symbols: int = 2000,
    device: str = "cuda" if torch.cuda.is_available() else "cpu",
):
    """Main training pipeline for 2000 stocks."""
    ...
    # 3. Normalization (compute on training data only)
    ...
    print(f"  Actions: HOLD={np.bincount(act_data, minlength=3)[0]:,} "
          f"BUY={np.bincount(act_data, minlength=3)[1]} "
          f"SELL={np.bincount(act_data, minlength=3)[2]}")
    ...
    model.save("experiment_6/models/pretrained_2000")
    print("[Pretrain] Model saved: experiment_6/models/pretrained_2000.zip")
    ...
    class EvalCallback:
        ...
        def evaluate(self, step):
            ...
            m = compute_metrics_2000(eq, trades, 100000.0)
    ...
    test_m = compute_metrics_2000(test_eq, test_trades, 100000.0)
    val_m = compute_metrics_2000(val_eq, val_trades, 100000.0)
    ...
    with open("experiment_6/results/final_results_2000.json", "w") as f:
        json.dump(results, f, indent=2, default=str)

    model.save("experiment_6/models/exp6_2000_final")
    print("\nModel saved: experiment_6/models/exp6_2000_final.zip")
...
def plot_2000_results(pretrain_hist, eval_hist, test_eq, val_eq, test_trades, val_trades, init=100000.0):
    """Generate comprehensive 9-panel plot."""
    ...
    # 7-9. ... (same as before)
    axes[2, 0].set_title("Placeholder"); axes[2, 0].text(0.5, 0.5, "2000-Stock Training", ha="center", fontsize=14)
    axes[2, 1].set_title("See full report"); axes[2, 2].set_title("experiment_6/results/")

    plt.suptitle("Experiment 6: 2000-Stock Enterprise RL Agent", fontsize=16, fontweight="bold")
    plt.tight_layout()
    plt.savefig("experiment_6/results/comprehensive_2000.png", dpi=150, bbox_inches="tight")
    plt.close()
    print("[Plot] Saved: experiment_6/results/comprehensive_2000.png")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Experiment 6 - 2000 Stock Training")
    ...
    if args.download:
        from experiment_6.download_2000 import download_sp500, read_tickers, download_stocks_bulk
        download_sp500("experiment_6/data/SP500_daily.csv")
        tickers = read_tickers(max_tickers=args.symbols)
        download_stocks_bulk(tickers, "experiment_6/data/stocks_2000_daily.parquet", max_workers=12)
