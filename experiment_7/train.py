"""
Experiment 7: Inside Bar Trend-Following RL Agent — Main Training Pipeline

Pipeline:
  1. Load & preprocess data
  2. Expert pretraining (behavioral cloning)
...
class MetricsCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.eval_freq == 0:
            ...
            if self.verbose:
                print(f"[{self.num_timesteps:>7d}] Ret={ret:+.2f}% Sharpe={sharpe:.2f} "
                      f"DD={dd:.1f}% Trades={len(trades)}")
...
def load_data():
    ...
    if "entry_gate" in df.columns:
        gates = df["entry_gate"].sum()
        print(f"  Entry gates active: {gates:,} ({gates/len(df)*100:.2f}% of bars)")
...
def split_data(df, train_r=0.7, val_r=0.15):
    ...
    for sym in sorted(df["Symbol"].unique()):
        ...
        n = len(sd)
        n1 = int(n * train_r)
        n2 = int(n * (train_r + val_r))
        train.append(sd.iloc[:n1])
        val.append(sd.iloc[n1:n2])
        test.append(sd.iloc[n2:])
...
def train_pipeline(run_optuna: bool = False, n_optuna_trials: int = 20,
    ...
    action_counts = np.bincount(act_data, minlength=2)
    print(f"  Expert actions: HOLD={action_counts[0]} BUY={action_counts[1]}")
    ...
    model.save("experiment_7/models/pretrained_policy")
    ...
    ckpt_cb = CheckpointCallback(save_freq=50000, save_path="experiment_7/checkpoints/", name_prefix="exp7")
    ...
    with open("experiment_7/results/final_results.json", "w") as f:
        json.dump(results, f, indent=2, default=str)

    model.save("experiment_7/models/exp7_final")
...
def plot_results(pretrain_hist, train_metrics, test_eq, val_eq,
    ...
    if pretrain_hist:
        ...
        ax.text(0.5, 0.5, "No pretrain data", ha="center")
    ...
    ax = axes[1, 2]
    if test_trades:
        pnls = [t.get("pnl_pct", t.get("pnl_usd", 0) / init * 100) for t in test_trades]
    ...
    plt.suptitle("Experiment 7: Inside Bar Trend-Following RL Agent — Report",
                 fontsize=16, fontweight="bold")
    plt.tight_layout()
    plt.savefig("experiment_7/results/comprehensive_report.png", dpi=150, bbox_inches="tight")
    plt.close()
    ...
    if train_metrics:
        ...
        ax.grid(alpha=0.3)
        plt.tight_layout()
        plt.savefig("experiment_7/results/training_curves.png", dpi=150, bbox_inches="tight")
        plt.close()
